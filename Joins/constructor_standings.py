...
year_filter = constructor_standings.filter(col("race_year") == 2020)
...
final_rank = year_filter.withColumn("rank", rank().over(standing_window))
...
final_rank.write.mode("overwrite").parquet(f"{presentation_folder_path}/constructor_final_rank")